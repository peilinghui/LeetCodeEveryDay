...
评论(24)
ming-tian-hui-tian-jingHHey
2020-06-01
C++版

void recur(vector<string>* &numbers, int n,char *num, int hi)
{
	string combine;
	for (int i = 0; i < 10; i++)
	{
		num[hi] = i + '0';
		for (int j = 0; j < n; j++)	combine += num[j];
		numbers->push_back(combine);
		combine = "";
	}
	string temp(n,'9' );
	for (int j = 0; j < n; j++)	combine += num[j];
	if (combine == temp)	return; 
	for (int i = 0; i < n; i++) 	//进位
	{
		if (num[hi] == '9')  num[hi-1] = char((int)num[hi - 1] + 1);
		hi -= 1;
	}
	recur(numbers, n, num, n-1);
}

vector<string> printNumbers02(int n)
{
	vector<string>* numbers = new vector<string>;
	char* num = new char[n] { '0' };
	recur(numbers, n, num, n - 1);
	delete[] num;
	return *numbers;
}